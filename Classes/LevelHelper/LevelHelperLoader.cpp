...
{
	return CCString::createWithFormat("{{%.2f, %.2f}, {%.2f, %.2f}}",rect.getMinX(),rect.getMinY(),rect.getMaxX(),rect.getMaxY());
}
CCString* LHStringWithCCPoint(CCPoint pt)
{
	return CCString::createWithFormat("{%.2f, %.2f}",pt.x,pt.y);
}
...
void LevelHelperLoader::convertScaleByScreen( CCPoint &scale, float angle )
{
	//scale.x *= LHSettings::sharedInstance()->convertRatio.x;
	//scale.y *= LHSettings::sharedInstance()->convertRatio.y;
	//Scale Recalc
	float x = LHSettings::sharedInstance()->convertRatio.x * cos(angle*M_PI/180);
	float y = LHSettings::sharedInstance()->convertRatio.y * sin(angle*M_PI/180);
	scale.x *= sqrtf(x*x+y*y);
	x = LHSettings::sharedInstance()->convertRatio.x * sin(angle*M_PI/180);
	y = LHSettings::sharedInstance()->convertRatio.y * cos(angle*M_PI/180);
...
void LevelHelperLoader::createAllAnimationsInfo()
{
    ...
    CCARRAY_FOREACH(lhAnims, obj)
    {
		...
		CCArray *ccframes = CCArray::create();        
        CCARRAY_FOREACH(framesInfo,obj)
        {
			...
			CCSpriteFrame* frame = CCSpriteFrame::createWithTexture(batch->getTexture(),rect);
...
{
	CCString* animName = CCString::createWithFormat("%s",spriteProp->valueForKey("AnimName")->getCString());
...
b2Body* LevelHelperLoader::leftPhysicBoundary(){
	return physicBoundarieForKey(CCString::create("LHPhysicBoundarieLeft"));
}
//------------------------------------------------------------------------------
b2Body* LevelHelperLoader::rightPhysicBoundary(){
	return physicBoundarieForKey(CCString::create("LHPhysicBoundarieRight"));
}
//------------------------------------------------------------------------------
b2Body* LevelHelperLoader::topPhysicBoundary(){
    return physicBoundarieForKey(CCString::create("LHPhysicBoundarieTop"));
}
//------------------------------------------------------------------------------
b2Body* LevelHelperLoader::bottomPhysicBoundary(){
    return physicBoundarieForKey(CCString::create("LHPhysicBoundarieBottom"));
}
//------------------------------------------------------------------------------
...
void LevelHelperLoader::createPathOnSprite(LHSprite* ccsprite ,CCDictionary* spriteProp){
	...
	CCString* uniqueName = CCString::createWithFormat("%s",spriteProp->valueForKey("PathName")->getCString());
    ...
    moveSprite(ccsprite,uniqueName ,pathSpeed , (bool)startPoint ,isCyclic ,pathOtherEnd,axisOrientation);
}
//------------------------------------------------------------------------------
...
void LevelHelperLoader::createSpritesWithPhysics() 
{
    ...
    CCARRAY_FOREACH(lhSprites,obj)
	{
		...
		if ( image->m_sString == "hero1_body.png" )
			image = CCString::createWithFormat("hero%d_body.png",g_nHeroSkinIndex+1);
		CCString *newImage = LHSettings::sharedInstance()->imagePath(image,false);
		...
		{
            LHSprite* ccsprite = spriteWithBatchFromDictionary(spriteProp,bNode);
			int zVal = ((CCString*)spriteProp->objectForKey("ZOrder"))->intValue();
			int ind = ccsprite->getUniqueName()->m_sString.find("box01");
			if ( ind >= 0 )
				zVal--;
			
            batch->addChild(ccsprite,zVal);
...
b2Body* LevelHelperLoader::b2BodyFromDictionary(CCDictionary* spritePhysic,CCDictionary*spriteProp,LHSprite*ccsprite ,b2World* _world)
{
    ...
    convertScaleByScreen( scale, ccsprite->getRotation() );
...
LHSprite* LevelHelperLoader::newSpriteWithUniqueName(CCString *name)
{
    ...
    CCARRAY_FOREACH(lhSprites,obj){
		...
		if(spriteProp->valueForKey("UniqueName")->m_sString.compare(name->m_sString)){            
			...
			CCString* uName = CCString::createWithFormat("%s_LH_NEW_SPRITE_%d", 
...
LHSprite* LevelHelperLoader::newPhysicalSpriteWithUniqueName(CCString* name)
{
	...
	CCARRAY_FOREACH(lhSprites,obj){
		...
		{            
			...
			CCString* uName = CCString::createWithFormat("%s_LH_NEW_BODY_%d", 
                               name->getCString(), LHSettings::sharedInstance()->getNewBodyId());
            ccsprite->setUniqueName(uName);
		...
		if (spriteProp->valueForKey("UniqueName")->m_sString.compare(name->m_sString)){            
            ...
            if(NULL != bNode)
			{
                ...
                if(NULL != batch){
					...
					CCString* uName = CCString::createWithFormat("%s_LH_NEW_BATCH_SPRITE_%d", 
                                       name->getCString(), LHSettings::sharedInstance()->getNewBodyId());
                    ccsprite->setUniqueName(uName);
                    return ccsprite;
                }
		...
		if (spriteProp->valueForKey("UniqueName")->m_sString.compare(name->m_sString)){            
			...
			if(NULL != bNode)
			{
                ...
                if(NULL != batch){
					...
					CCString* uName = CCString::createWithFormat("%s_LH_NEW_BATCH_BODY_%d", 
                                       name->getCString(), LHSettings::sharedInstance()->getNewBodyId());
                    ccsprite->setUniqueName(uName);
                    return ccsprite;
                }
...
LHSprite* LevelHelperLoader::spriteFromDictionary(CCDictionary* spriteProp)
{
	...
	CCString* img = CCString::createWithFormat("%s",LHSettings::sharedInstance()->imagePath((CCString*)spriteProp->valueForKey("Image"))->getCString());
...
LHSprite* LevelHelperLoader::spriteWithBatchFromDictionary(CCDictionary* spriteProp ,LHBatch* lhBatch)
{
	...
	//Resolution Control Code
	uv.origin.x = iDevPixelX(uv.origin.x);
	uv.origin.y = iDevPixelY(uv.origin.y);
	uv.size.width = iDevPixelX(uv.size.width);
	uv.size.height = iDevPixelY(uv.size.height);
	//////////////////////////////////////////////////////////////////////////
...
void LevelHelperLoader::setSpriteProperties(LHSprite* ccsprite, CCDictionary* spriteProp)
{
	...
	//Resolution Control Code
	scale.x = scale.x * 1024 / VIRT_WIDTH;
	scale.y = scale.y * 768 / VIRT_HEIGHT;
	//////////////////////////////////////////////////////////////////////////
...
LHParallaxNode* LevelHelperLoader::parallaxNodeFromDictionary(CCDictionary* parallaxDict,CCLayer* layer)
{
    ...
    CCARRAY_FOREACH(spritesInfo,obj){
		...
		CCString* sprName = CCString::createWithFormat("%s",sprInf->valueForKey("SpriteName")->getCString());
...
void LevelHelperLoader::loadLevelHelperSceneFile(CCString* levelFile,CCString* subfolder,CCString* imgFolder)
{
	...
	CCString *path = CCString::createWithFormat("%s%s%s.plhs",subfolder->getCString(),imgFolder->getCString(),levelFile->getCString());
	...
	CCDictionary *dictionary = CCDictionary::createWithContentsOfFile(realPath);
...
void LevelHelperLoader::processLevelFileFromDictionary(CCDictionary* dictionary)
{
	...
	CCARRAY_FOREACH(batchImages,obj)
	{
		...
		CCSpriteBatchNode *batch = CCSpriteBatchNode::create(LHSettings::sharedInstance()->imagePath(image)->getCString());
